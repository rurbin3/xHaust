...
module.exports = class Ui extends require('../classes/package') {
  constructor() {
    super()
  }
  ...
  async load() {
    ...
    var grid = new contrib.grid({ rows: 24, cols: 24, screen: this.screen })
    this.grid = {
      logs: grid.set(0, 0, 24, 10, blessed.box, { label: 'Logs' }),
      throughput: grid.set(0, 10, 3, 9, blessed.box, { label: 'Bandwidth' }),
      status: grid.set(0, 19, 10, 5, blessed.box, { label: 'Status' })
    }

    const components = {}

    components['latency'] = grid.set(8, 8, 4, 2, contrib.line, {
    ...
    // Status Component
    components['status'] = contrib.donut({
      appendTo: 'grid.status',
      label: 'Test',
      radius: 20,
      arcWidth: 3,
      remainColor: 'black',
    ...
    // // Spark Component
    components['spark'] = contrib.sparkline({
      appendTo: 'grid.throughput',
      label: '',
      tags: true,
      style: { fg: 'blue' }
    })
    ...
    // // Gauge component
    // components['gauge'] = contrib.gauge({
    //   appendTo: 'grid.g2',
    //   label: 'Progress',
    //   stroke: 'green',
    //   fill: 'white'
    ...
    for (let name in components) {
      ...
      const cmd = eval(`this.${component.options.appendTo}`)
  ...
  fakeData() {
    ...
    this.debug.log('Some log text: ' + String(new Date() / 1000))

    // // fake linebar chart
    // this.components.line.series.push({
    //   title: 'Requests per/sec',
    //   x: ['t1', 't2', 't3', 't4'],
    //   y: [5, 1, 7, 5]
    // })
    // this.components.line.setData(this.components['line'].series)

    // // Fake spark
    if (!this.components.spark.series) {
      this.components.spark.series = [new Array(128).fill(0)]
    }
    this.components.spark.series[0] = [...Array(128)].map(() => Math.floor(Math.random() * 9))
    this.components.spark.setData(['kb/s'], this.components.spark.series)

    // status data
    if (!this.components['status'].percent) {
      this.components['status'].percent = 0
    }
    this.components['status'].percent++
    this.components['status'].setData([{ percent: this.components['status'].percent, label: 'rcp', color: 'green' }])

    setTimeout(this.fakeData.bind(this), 100)
  }
...
*/
    ...
    var log = grid.set(8, 6, 4, 2, contrib.log, { fg: 'green', selectedFg: 'green', label: 'Server Log' })
    ...
    function updateDonut() {
      ...
      donut.setData([{ percent: parseFloat((pct + 0.0) % 1).toFixed(2), label: 'storage', color: color }])
      pct += 0.01
    }
